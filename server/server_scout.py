...
def get_scout_view(person_email,person_id):
    pass
...
while True:
    ...
    with conn:
        cursor.execute("""SELECT propic,feed_thumbnail FROM accounts
                    WHERE id = (:person_id)""",{
                    "person_id":person_id
                    })
        person_propic,thumbnail = cursor.fetchone()
    print(os.getcwd())
    pro_and_thu = [PilImage.open(person_propic),PilImage.open(thumbnail)]

    print("sending propic and thumbanil")
    with open('pro_and_thu.pickle','wb') as f:
        pickle.dump(pro_and_thu,f)
    with open('pro_and_thu.pickle','rb') as f:
        data = f.read(4096)
        while data:
            c.sendall(data)
            data = f.read(4096)
